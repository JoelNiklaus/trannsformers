...
def run(base_model="roberta-base", fine_tuned_checkpoint=None, dataset="joelito/sem_eval_2010_task_8",
        num_train_epochs=3, do_train=False, do_eval=False, do_predict=True, batch_size=14, test_set_sub_size=10,
        seed=42):
    """
    ...
    :param fine_tuned_checkpoint:  the name of the fine tuned checkpoint (e.g. checkpoint-500)
    ...
    :param batch_size:             size of the batch (dependent on gpu: e.g. bert-base => 6, distilbert-base => 14 on 8GB)
    ...
    training_args = TrainingArguments(
        output_dir=f'{local_model_name}/results/{seed}',  # output directory
        ...
        logging_dir=f'{local_model_name}/logs/{seed}',  # directory for storing logs
        logging_steps=10,
        save_steps=500,
        eval_steps=100,
        evaluation_strategy=EvaluationStrategy.STEPS,
        seed=seed,
        run_name=f"{base_model}/{seed}",  # used for wandb
    ...
    if fine_tuned_checkpoint:
        model_path = f"{training_args.output_dir}/{fine_tuned_checkpoint}"
